...
public class UserActivityFacade {
    ...
    public UserActivityList findReservationListByUserId(Long userId, int page, int pageSize) {
        Page<ReservationActivity> reservationList = reservationService.findReservationListByUserId(userId, page, pageSize);
        List<Integer> contentIds = reservationService.getReservationActivityInquiryIds(reservationList.getContent());

        Map<Integer, List<String>> fileNameListMap = spaceService.findByContentIds(contentIds);

        List<UserActivity> userActivityList = reservationService.reservationActivityToUserActivityList(reservationList.getContent(), fileNameListMap);
        Pagination pagination = new Pagination(page, pageSize, reservationList.getTotalElements(), reservationList.getTotalPages());
