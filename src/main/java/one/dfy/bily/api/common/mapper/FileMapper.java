...
public interface FileMapper {
    ...
    // 공간 이미지 파일 업로드
    @Insert("INSERT INTO bilydatabase.TBL_SPACE_FILE_INFO (CONTENT_ID, FILE_NAME, SAVE_FILE_NAME, SAVE_LOCATION, SAVE_SIZE, CREATE_DATE, CREATOR, DELETE_FLAG, UPDATER, FILE_TYPE) VALUES (#{contentId}, #{fileName}, #{saveFileName}, #{saveLocation}, #{saveSize}, #{createDate}, #{creator}, #{deleteFlag}, #{updater}, #{fileType})")
    ...
    @Select(
            "SELECT " +
                    ...
                    "FROM " +
                    "  bilydatabase.TBL_SPACE_FILE_INFO " +
                    "WHERE " +
                    "  CONTENT_ID = #{contentId}"
    )
    List<FileInfoEntity> selectAllFiles(Long contentId);

    @Select("SELECT * FROM bilydatabase.TBL_SPACE_FILE_INFO WHERE ATTACH_FILE_ID = #{attachFileId}")
    FileInfoEntity selectFileById(Long attachFileId);

    @Delete("DELETE FROM bilydatabase.TBL_SPACE_FILE_INFO WHERE ATTACH_FILE_ID = #{attachFileId}")
    void deleteFile(Long attachFileId);

    @Update("UPDATE bilydatabase.TBL_SPACE_FILE_INFO SET FILE_NAME = #{fileName}, SAVE_FILE_NAME = #{saveFileName}, SAVE_LOCATION = #{saveLocation}, SAVE_SIZE = #{saveSize}, CREATE_DATE = #{createDate}, CREATOR = #{creator} WHERE ATTACH_FILE_ID = #{attachFileId}")
